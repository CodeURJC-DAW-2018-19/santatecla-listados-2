...
cd ..
cd angular
docker run --rm --name angular-cli -v ${PWD}:/angular -w /angular node:8.15.1 /bin/bash -c "npm install -g @angular/cli; ng config -g cli.warnings.versionMismatch false; npm install; ng build --prod --baseHref=/new/"cd target
